	...
	render_scene(context, program_state, shadow_pass, draw_light_source=false, draw_shadow=false) {
		...
		for (let i=0; i < xlen; i++) {
			...
			for (let j=0; j < zlen; j++) {
				// wall
				if (gl[j] == 1) {
					this.shapes.Tree_Trunks.model.draw(context, program_state, Mat4.translation(2*i, 0, 2*j), shadow_pass ? this.shapes.Tree_Trunks.material : this.pure);
					this.shapes.Tree_Leaves.model.draw(context, program_state, Mat4.translation(2*i, 2, 2*j), shadow_pass ? this.shapes.Tree_Leaves.material : this.pure);
				}
				// crate
